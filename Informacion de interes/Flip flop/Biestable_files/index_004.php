...
if (wgNamespaceNumber==-1 || wgNamespaceNumber==6 || wgNamespaceNumber==7 || wgNamespaceNumber==8) {
   ...
   for (var i=paginasExcluidas.length; i>=0; i--) {
      if (wgPageName==paginasExcluidas[i]) {
          importStylesheet('MediaWiki:Wikibugs.css')
          break
      }
   }
...
{
  var link_wiki = window.wgArticlePath.replace(/\$1/, 'wiki')
  var link_tocreate = window.wgArticlePath.replace(/\$1/, 'Wikipedia:Artículos_solicitados')
  var link_bebold = window.wgArticlePath.replace(/\$1/, 'Wikipedia:Sé_valiente_editando_páginas')
  var link_buglist = window.wgArticlePath.replace(/\$1/, 'Wikipedia:Informes_de_error')
  ...
  wb$popBugBoth("Wikipedia:Informes de error", '\
    ...
    <p style="margin-top:0">Si has encontrado un error, por favor, intenta arreglarlo tú mismo, la <a href="' + link_wiki + '" target="_blank">tecnología wiki</a> permite que cualquiera pueda <strong><a href="http://es.wikipedia.org/wiki/Ayuda:Introducción" target="_blank">editar artículos</a></strong>.</p>\
    ...
    <p>Lee los <a href="http://wikimediafoundation.org/wiki/Términos_de_Uso" target="_blank">términos de uso</a> y la <a href="http://wikimediafoundation.org/wiki/Política_de_privacidad" target="_blank">política de privacidad</a>.</p>\
    ')
  return false
}
...
function wb$checkForm(form)
{
  ...
  page = page.replace(/^http:\/\/es\.wikipedia\.org\/wiki\/(.+)$/, "$1")
...
function wb$goToEditPage()
{
  ...
  var edit_href = window.wgArticlePath.replace(/\$1/, 'Wikipedia:Informes_de_error')
  if (edit_el)
...
function wb$popBugBoth(action_page, infoHTML)
{
  ...
  if (window.wgUserName == null)
    nel.innerHTML = nel.innerHTML + '<p><b>Atención.</b> Tu dirección IP quedará registrada en el historial y podrá verse. Puedes crearte una <a href="http://es.wikipedia.org/w/index.php?title=Especial:Entrar&type=signup" target="_blank">cuenta de usuario</a>.</p>'
